...
class XMLInclude():
			...
			if tag == "font" and "size" in tags:
				...
				if len(font_parts) > 1:
					...
					if float(size_height) < float(font_size) * 4.0 / 3.0:
						print("WARNING: size: %s < font: %s" % (size_height, float(font_size) * 4.0 / 3.0))
	...
	def getIncFilePath(self, inc_filename):
		...
		if not os.path.exists(inc_file):
			...
			copy1 = "cp %s %s" % (inc_file, os.path.join(self.dstdir, inc_filename))
			copy2 = noop
			do_scale = False
			do_delete = False
			# print("inc_file: %s" % inc_file)
			...
			if not os.path.exists(inc_file):
				inc_file = os.path.join(os.path.dirname(self.dstdir), inc_filename)
				copy1 = noop
				copy2 = "cp %s %s" % (inc_file, os.path.join(self.dstdir, inc_filename))
				# print("inc_file: %s" % inc_file)
				if not os.path.exists(inc_file):
					inc_file = os.path.join(os.path.dirname(self.cmndir), inc_filename)
					copy1 = "cp %s %s" % (inc_file, os.path.join(os.path.dirname(self.dstdir), inc_filename))
					copy2 = "cp %s %s" % (os.path.join(os.path.dirname(self.dstdir), inc_filename), os.path.join(self.dstdir, inc_filename))
					# print("inc_file: %s" % inc_file)
					if not os.path.exists(inc_file):
	...
	def processFile(self, level, olines, afile, pos, do_delete):
		print("==> processFile: >%s, (%s,%s), %s, do_delete: %s" % (level, pos.x, pos.y, afile, do_delete))
			...
			if "layout" in tags:
				...
				if "layout" in tags and tags["layout"] == "/>":
					...
					if not tags["name"] in self.layouts:
						print("==> processFile: >%s, ERROR: layout %s not defined" % (level, tags["name"]))
					print("==> processFile: >%s, >>> including layout %s" % (level, tags["name"]))
			if "xmlinc" in tags:
				...
				if os.path.basename(inc_file).startswith("applet_"):
					...
					self.processFile(level + 1, olines, inc_file, pos2, next_delete)
...
def xmlinc(argv):
	...
	if len(opts) < 4:
		print('Usage: python xmlinc.py -i <srcinfile> -o <srcoutfile> -d <dstdir> -c <cmndir>')
		sys.exit(2)
	...
	olines = []
	XMLInclude(srcdir, dstdir, cmndir).processFile(0, olines, "skin_src.xml", Pos(0, 0), False)
	output = "\n".join(olines) + "\n"
